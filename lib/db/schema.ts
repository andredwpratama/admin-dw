...
export const metrics = sqliteTable("metrics", {
  ...
  spend: real("spend").notNull().default(0),   // IDR
  conversions: integer("conversions").notNull().default(0),
  revenue: real("revenue"),                    // IDR, optional
  // computed fields for query performance
  ctr: real("ctr").notNull().default(0),       // clicks / impressions * 100
  cpc: real("cpc").notNull().default(0),       // spend / clicks
  cpa: real("cpa").notNull().default(0),       // spend / conversions
  roas: real("roas"),                          // revenue / spend
}, (table) => ({
...
export const genericData = sqliteTable("generic_data", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
